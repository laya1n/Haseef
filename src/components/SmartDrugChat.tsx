// src/components/SmartDrugChat.tsx
...
const SmartDrugChat: React.FC<SmartDrugChatProps> = ({
  ...
  const [msgs, setMsgs] = useState<ChatMsg[]>([
    { id: "m1", role: "assistant", text: "كيف أساعدك في السجلات الدوائية؟" },
  ]);
  ...
  // ✅ أسئلة جاهزة للسجلات الدوائية
  const QUICK_ITEMS: { label: string; query: string }[] = [
    {
      label: "إجمالي عدد عمليات صرف الدواء هذا الشهر",
      query: `from:${monthBounds.from} to:${monthBounds.to} metric:dispense_count`,
    },
    {
      label: "الأدوية الأكثر صرفًا هذا الشهر",
      query: `from:${monthBounds.from} to:${monthBounds.to} metric:top_drugs`,
    },
    {
      label: "الوصفات التي تحتوي أكثر من دواءين",
      query: `drugs_count>2`,
    },
    {
      label: "الوصفات ذات التنبيهات (جرعة/تداخل)",
      query: `flag:alert`,
    },
    {
      label: "وصفات اليوم",
  ...
  const applyQuick = (item: { label: string; query: string }) => {
    ...
    // ⬇️ نضيف الرسالتين مرة واحدة
    setMsgs((m) => [...m, userMsg, botMsg]);

    // إشعار صفحة السجلات الدوائية لتشغيل البحث
    try {
      window.dispatchEvent(
        new CustomEvent("drug:runQuick", { detail: { query: item.query } })
      );
    } catch {
      // لا شيء
    }
  ...
  return (
    <>
      ...
      <button
        ...
        aria-label="المساعد الدوائي"
      >
        <Bot className="size-5" />
        <span className="font-semibold hidden sm:inline">المساعد الدوائي</span>
      </button>
        ...
        >
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div className="font-semibold text-gray-800">المساعد الدوائي</div>
            </div>
            <div className="flex items-center gap-2">
              <button
          ...
          >
            <div className="text-[12px] text-emerald-900/80 mb-1">
              الأسئلة الجاهزة (السجلات الدوائية)
            </div>
          ...
          >
            ...
            <textarea
              ...
              placeholder="اكتب استفسارك الدوائي..."
